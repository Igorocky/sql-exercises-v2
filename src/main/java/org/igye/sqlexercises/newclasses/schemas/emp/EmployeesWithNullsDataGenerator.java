...
@Service
public class EmployeesWithNullsDataGenerator extends TestDataGenerator {
    public static final String RANDOM_DATA = "random-data";
    private List<String> firstNames;
    private List<String> lastNames;

    public EmployeesWithNullsDataGenerator() throws IOException {
    ...
    @Override
    public String getId() {
        return "employees-with-nulls";
    }

    @Override
    public List<String> generateTestData() throws Exception {
        if (1 == 1) {
            return readLines("schemas/employees-with-nulls-data.sql");
        } else {
            List<String> res = new ArrayList<>();
            for (int i = 0; i < 100; i++) {
                Integer depId = rnd.nextInt(6) + 1;
                depId = depId == 6 ? null : depId;
